...
(function (root, factory) {
  if (typeof define === 'function' && define.amd) { define('p5.sound', ['p5'], (p5) => { (factory(p5)); }); } else if (typeof exports === 'object') { factory(require('../p5')); } else { factory(root.p5); }
}(this, (p5) => {
  let sndcore;
  'use strict';
  sndcore = (function () {
  ...
  */
   ...
   */
    ...
    const isOGGSupported = function () {
      return !!el.canPlayType && el.canPlayType('audio/ogg; codecs="vorbis"');
    };
    const isMP3Supported = function () {
      return !!el.canPlayType && el.canPlayType('audio/mpeg;');
    };
    const isWAVSupported = function () {
      return !!el.canPlayType && el.canPlayType('audio/wav; codecs="1"');
    };
    const isAACSupported = function () {
      return !!el.canPlayType && (el.canPlayType('audio/x-m4a;') || el.canPlayType('audio/aac;'));
    };
    const isAIFSupported = function () {
   ...
   */
    p5.prototype.masterVolume = function (vol, rampTime, tFromNow) {
      if (typeof vol === 'number') {
        ...
        const currentVol = p5sound.output.gain.value;
  ...
  /*
     ...
     */
    const CustomError = function (name, errorTrace, failedPath) {
      const err = new Error();
      ...
      splitStack = splitStack.filter((ln) => {
  ...
  panner = (function () {
    ...
    // use this method to create the effect
      ...
      p5.Panner.prototype.pan = function (val, tFromNow) {
        ...
        const t = ac.currentTime + time;
        const v = (val + 1) / 2;
        const rightVal = Math.cos(v * Math.PI / 2);
        const leftVal = Math.sin(v * Math.PI / 2);
    ...
    p5.SoundFile = function (paths, onload, onerror, whileLoading) {
      if (typeof paths !== 'undefined') {
        if (typeof paths === 'string' || typeof paths[0] === 'string') {
          const path = p5.prototype._checkFileFormats(paths);
          this.url = path;
        } else if (typeof paths === 'object') {
   ...
   */
    ...
    // if loading locally without a server
      ...
      const self = this;
      const s = new p5.SoundFile(path, function () {
   ...
   */
    p5.SoundFile.prototype.load = function (callback, errorCallback) {
      ...
      if (this.url !== undefined && this.url !== '') {
        ...
        request.onload = function () {
          ...
          // on sucess loading file:
            ac.decodeAudioData(
              ...
              () => {
                const err = new CustomError('decodeAudioData', errorTrace, self.url);
                const msg = `AudioContext error at decodeAudioData for ${self.url}`;
                if (errorCallback) {
                  err.msg = msg;
                  errorCallback(err);
                } else {
            ...
            const err = new CustomError('loadSound', errorTrace, self.url);
            const msg = `Unable to load ${self.url}. The request status was: ${request.status} (${request.statusText})`;
            if (errorCallback) {
              err.message = msg;
              errorCallback(err);
            } else {
        ...
        request.onerror = function () {
          const err = new CustomError('loadSound', errorTrace, self.url);
          const msg = `There was no response from the server at ${self.url}. Check the url and internet connectivity.`;
          if (errorCallback) {
            err.message = msg;
            errorCallback(err);
          } else {
        ...
        reader.onload = function () {
          ac.decodeAudioData(reader.result, (buff) => {
            self.buffer = buff;
            self.panner.inputChannels(buff.numberOfChannels);
            if (callback) {
              callback(self);
    ...
    p5.SoundFile.prototype._updateProgress = function (evt) {
      if (evt.lengthComputable) {
        const percentComplete = evt.loaded / evt.total * 0.99;
    ...
    p5.SoundFile.prototype.play = function (startTime, rate, amp, _cueStart, duration) {
      ...
      // reset the pause time (if it was paused)
        ...
        var clearOnEnd = function () {
          ...
          self._onended(self);
          self.bufferSourceNodes.forEach((n, i) => {
            if (n._playing === false) {
      ...
      if (this._looping === true) {
        cueEnd = duration || cueStart - 1e-15;
   ...
   */
    p5.SoundFile.prototype.setVolume = function (vol, _rampTime, _tFromNow) {
      if (typeof vol === 'number') {
        const rampTime = _rampTime || 0;
        const tFromNow = _tFromNow || 0;
        const now = p5sound.audiocontext.currentTime;
   ...
   */
    ...
    p5.SoundFile.prototype.setPitch = function (num) {
      const newPlaybackRate = midiToFreq(num) / midiToFreq(60);
  ...
  */
    ...
    p5.SoundFile.prototype.getPeaks = function (length) {
      ...
      // set length to window's width if no length is provided
        ...
        if (this.buffer) {
          ...
          const peaks = new Float32Array(Math.round(length));
   ...
   */
    p5.SoundFile.prototype.reverseBuffer = function () {
      if (this.buffer) {
        const currentPos = this._lastPos / ac.sampleRate;
        const curVol = this.getVolume();
   ...
   */
    p5.SoundFile.prototype.setPath = function (p, callback) {
      const path = p5.prototype._checkFileFormats(p);
      this.url = path;
      this.load(callback);
    };
    /**
   ...
   */
    p5.SoundFile.prototype.setBuffer = function (buf) {
      const numChannels = buf.length;
      const size = buf[0].length;
      const newBuffer = ac.createBuffer(numChannels, size, ac.sampleRate);
      if (!(buf[0] instanceof Float32Array)) {
        buf[0] = new Float32Array(buf[0]);
      }
      for (let channelNum = 0; channelNum < numChannels; channelNum++) {
        const channel = newBuffer.getChannelData(channelNum);
    ...
    // //////////////////////////////////////////////////////////////////////////////////
    const _createCounterBuffer = function (buffer) {
    ...
    p5.SoundFile.prototype._initCounterNode = function () {
      ...
      const cNode = ac.createBufferSource();
      ...
      self._scopeNode.onaudioprocess = function (processEvent) {
        const inputBuffer = processEvent.inputBuffer.getChannelData(0);
    ...
    p5.SoundFile.prototype._initSourceNode = function () {
      const bufferSourceNode = ac.createBufferSource();
   ...
   */
    p5.SoundFile.prototype.processPeaks = function (callback, _initThreshold, _minThreshold, _minPeaks) {
      ...
      const offlineContext = new window.OfflineAudioContext(1, bufLen, sampleRate);
      ...
      offlineContext.oncomplete = function (e) {
        ...
        const intervalCounts = countIntervalsBetweenNearbyPeaks(allPeaks);
        // step 3: find top tempos
        const groups = groupNeighborsByTempo(intervalCounts, filteredBuffer.sampleRate);
        // sort top intervals
        const topTempos = groups.sort((intA, intB) => {
        ...
        const bpmVariance = 5;
        const tempoPeaks = getPeaksAtTopTempo(allPeaks, topTempos[0].tempo, filteredBuffer.sampleRate, bpmVariance);
        callback(tempoPeaks);
      };
    };
    // process peaks
    const Peak = function (amp, i) {
      this.sampleIndex = i;
      this.amplitude = amp;
      this.tempos = [];
      this.intervals = [];
    ...
    function countIntervalsBetweenNearbyPeaks(peaksObj) {
      ...
      const peaksArray = Object.keys(peaksObj).sort();
      for (let index = 0; index < peaksArray.length; index++) {
      // find intervals in comparison to nearby peaks
        for (let i = 0; i < 10; i++) {
          const startPeak = peaksObj[peaksArray[index]];
          const endPeak = peaksObj[peaksArray[index + i]];
          if (startPeak && endPeak) {
            const startPos = startPeak.sampleIndex;
            const endPos = endPeak.sampleIndex;
    ...
    function getPeaksAtTopTempo(peaksObj, tempo, sampleRate, bpmVariance) {
      ...
      for (let i = 0; i < peaksArray.length; i++) {
        const key = peaksArray[i];
        const peak = peaksObj[key];
      ...
      peaksAtTopTempo = peaksAtTopTempo.filter((peakTime, index, arr) => {
        const dif = arr[index + 1] - peakTime;
        if (dif > 0.01) {
          return true;
        }
      });
   ...
   */
    p5.SoundFile.prototype.removeCue = function (id) {
      const cueLength = this._cues.length;
   ...
   */
    ...
    p5.SoundFile.prototype._onTimeUpdate = function (position) {
      const playbackTime = position / this.buffer.sampleRate;
      const cueLength = this._cues.length;
      for (let i = 0; i < cueLength; i++) {
        const cue = this._cues[i];
        const callbackTime = cue.time;
        const val = cue.val;
   ...
   */
    ...
    p5.Amplitude.prototype._audioProcess = function (event) {
      ...
      const self = this;
      const volSum = this.stereoVol.reduce((previousValue, currentValue, index) => {
    ...
    p5.FFT.prototype.waveform = function () {
      ...
      if (mode && !p5.prototype._isSafari()) {
        ...
        for (let j = 0; j < this.timeDomain.length; j++) {
          const scaled = p5.prototype.map(this.timeDomain[j], 0, 255, -1, 1);
   ...
   */
    p5.FFT.prototype.analyze = function () {
      ...
      if (mode && mode.toLowerCase() === 'db') {
        ...
        const normalArray = Array.apply([], this.freqDomain);
   ...
   */
    p5.FFT.prototype.getEnergy = function (frequency1, frequency2) {
      const nyquist = p5sound.audiocontext.sampleRate / 2;
      if (frequency1 === 'bass') {
      ...
      // if second is higher than first
        ...
        const lowIndex = Math.round(frequency1 / nyquist * this.freqDomain.length);
        const highIndex = Math.round(frequency2 / nyquist * this.freqDomain.length);
        let total = 0;
    ...
    p5.FFT.prototype.getFreq = function (freq1, freq2) {
      ...
      const x = this.getEnergy(freq1, freq2);
      return x;
    };
    /**
     *  Returns the
    ...
    p5.FFT.prototype.getCentroid = function () {
      ...
      const spec_centroid_freq = mean_freq_index * (nyquist / this.freqDomain.length);
   ...
   */
    p5.FFT.prototype.logAverages = function (octaveBands) {
      ...
      let octaveIndex = 0;
      for (let specIndex = 0; specIndex < spectrumLength; specIndex++) {
        const specIndexFrequency = Math.round(specIndex * nyquist / this.freqDomain.length);
  ...
  Tone_core_Tone = (function () {
    ...
    const Tone = function (inputs, outputs) {
        ...
        if (attr.indexOf('.') !== -1) {
          ...
          for (let i = 0; i < attrSplit.length - 1; i++) {
            ...
            if (parent instanceof Tone) {
              ...
              const innerParam = attrSplit.join('.');
    ...
    Tone.prototype._collectDefaults = function (constr) {
      ...
      if (!this.isUndef(constr._super)) {
        const superDefs = this._collectDefaults(constr._super);
    ...
    Tone.prototype.toString = function () {
      for (const className in Tone) {
        const isLetter = className[0].match(/^[A-Z]$/);
        const sameConstructor = Tone[className] === this.constructor;
    ...
    Object.defineProperty(Tone, 'supported', {
      get() {
        ...
        const hasWorkers = window.hasOwnProperty('Worker');
  ...
  Tone_type_TimeBase = (function (Tone) {
    Tone.TimeBase = function (val, units) {
      if (this instanceof Tone.TimeBase) {
        ...
        if (val instanceof Tone.TimeBase) {
          ...
          const method = this._primaryExpressions[units].method;
    ...
    Tone.TimeBase.prototype._tokenize = function (expr) {
      ...
      while (expr.length > 0) {
        expr = expr.trim();
        const token = getNextToken(expr, this);
        tokens.push(token);
      ...
      function getNextToken(expr, context) {
        ...
        for (let i = 0; i < expressions.length; i++) {
          const group = context[expressions[i]];
          for (const opName in group) {
            const op = group[opName];
            const reg = op.regexp;
    ...
    Tone.TimeBase.prototype._parseUnary = function (lexer) {
      let token, expr;
      token = lexer.peek();
      const op = this._matchGroup(token, this._unaryExpressions);
      if (op) {
        token = lexer.next();
        expr = this._parseUnary(lexer);
    ...
    Tone.TimeBase.prototype._parsePrimary = function (lexer) {
      ...
      if (this._matchGroup(token, this._primaryExpressions)) {
        token = lexer.next();
        const matching = token.value.match(token.regexp);
    ...
    Tone.TimeBase.prototype._parseExprString = function (exprString) {
      ...
      const lexer = this._tokenize(exprString);
      const tree = this._parseBinary(lexer);
      return tree;
    };
  ...
  Tone_type_Time = (function (Tone) {
    ...
    Tone.Time.prototype.quantize = function (subdiv, percent) {
      ...
      this._expr = function (expr, subdivision, percent) {
        ...
        const multiple = Math.round(expr / subdivision);
        const ideal = multiple * subdivision;
        const diff = ideal - expr;
    ...
    Tone.Time.prototype.toNotation = function () {
      ...
      const retNotation = this._toNotationHelper(time, testNotations);
      const testTripletNotations = [
        '1m',
        '2n',
        '2t',
        '4n',
      ...
      const retTripletNotation = this._toNotationHelper(time, testTripletNotations);
    ...
    Tone.Time.prototype._toNotationHelper = function (units, testNotations) {
      const threshold = this._notationToUnits(testNotations[testNotations.length - 1]);
      let retNotation = '';
      for (let i = 0; i < testNotations.length; i++) {
        const notationTime = this._notationToUnits(testNotations[i]);
        let multiple = units / notationTime;
        const floatingPointError = 0.000001;
    ...
    Tone.Time.prototype._notationToUnits = function (notation) {
      const primaryExprs = this._primaryExpressions;
      const notationExprs = [
        primaryExprs.n,
        primaryExprs.t,
        primaryExprs.m,
      ...
      for (let i = 0; i < notationExprs.length; i++) {
        const expr = notationExprs[i];
        const match = notation.match(expr.regexp);
        if (match) {
    ...
    Tone.Time.prototype.toBarsBeatsSixteenths = function () {
      ...
      const measures = Math.floor(quarters / this._timeSignature());
  ...
  Tone_type_Frequency = (function (Tone) {
    ...
    Tone.Frequency.prototype.toNote = function () {
      ...
      const log = Math.log(freq / Tone.Frequency.A4) / Math.LN2;
  ...
  Tone_type_TransportTime = (function (Tone) {
    ...
    Tone.TransportTime.prototype._unaryExpressions.quantize = {
      regexp: /^@/,
      method(rh) {
        const subdivision = this._secondsToTicks(rh());
        const multiple = Math.ceil(Tone.Transport.ticks / subdivision);
    ...
    Tone.TransportTime.prototype.valueOf = function () {
      const val = this._secondsToTicks(this._expr());
  ...
  Tone_core_Emitter = (function (Tone) {
    ...
    Tone.Emitter.mixin = function (object) {
      ...
      for (let i = 0; i < functions.length; i++) {
        ...
        const emitterFunc = Tone.Emitter.prototype[func];
  ...
  Tone_core_Context = (function (Tone) {
    ...
    Tone.Context = function (context) {
      ...
      for (const prop in this._context) {
    ...
    Tone.Context.prototype._createWorker = function () {
      ...
      const blob = new Blob([`var timeoutTime = ${(this._updateInterval * 1000).toFixed(1)};` + 'self.onmessage = function(msg){' + '\ttimeoutTime = parseInt(msg.data);' + '};' + 'function tick(){' + '\tsetTimeout(tick, timeoutTime);' + '\tself.postMessage(\'tick\');' + '}' + 'tick();']);
  ...
  Tone_core_Param = (function (Tone) {
    ...
    Tone.Param = function () {
      const options = this.optionsObject(arguments, [
        'param',
        'units',
        'convert',
      ], Tone.Param.defaults);
    ...
    Object.defineProperty(Tone.Param.prototype, 'value', {
      ...
      set(value) {
        if (this.isObject(value)) {
          ...
          const convertedVal = this._fromUnits(value);
    ...
    Tone.Param.prototype.setRampPoint = function (now) {
      ...
      let currentVal = this._param.value;
      if (currentVal === 0) {
  ...
  // https://github.com/TONEnoTONE/Tone.js/
    const Signal = Tone_signal_Signal;
    const Add = Tone_signal_Add;
    const Mult = Tone_signal_Multiply;
    const Scale = Tone_signal_Scale;
    const Tone = Tone_core_Tone;
    const p5sound = master;
   ...
   */
    Signal.prototype.scale = function (inMin, inMax, outMin, outMax) {
      ...
      const scale = new Scale(mapOutMin, mapOutMax);
      this.connect(scale);
      return scale;
    };
   ...
   */
    p5.Oscillator.prototype.phase = function (p) {
      const delayAmt = p5.prototype.map(p, 0, 1, 0, 1 / this.f);
      const now = p5sound.audiocontext.currentTime;
      this.phaseAmount = p;
      if (!this.dNode) {
      // create a delay node
    ...
    const sigChain = function (o, mathObj, thisChain, nextChain, type) {
   ...
   */
    p5.Oscillator.prototype.add = function (num) {
      const add = new Add(num);
      const thisChain = this.mathOps.length - 1;
      const nextChain = this.output;
  ...
  /** Tone.js module by Yotam Mann, MIT License 2016  http://opensource.org/licenses/MIT * */
  let Tone_core_Timeline;
  Tone_core_Timeline = (function (Tone) {
    'use strict';

    Tone.Timeline = function () {
      const options = this.optionsObject(arguments, ['memory'], Tone.Timeline.defaults);
      this._timeline = [];
      this._toRemove = [];
    ...
    Tone.Timeline.prototype._iterate = function (callback, lowerBound, upperBound) {
      ...
      if (this._toRemove.length > 0) {
        for (let j = 0; j < this._toRemove.length; j++) {
          const index = this._timeline.indexOf(this._toRemove[j]);
          if (index !== -1) {
            this._timeline.splice(index, 1);
          }
        }
  ...
  /** Tone.js module by Yotam Mann, MIT License 2016  http://opensource.org/licenses/MIT * */
  let Tone_signal_TimelineSignal;
  Tone_signal_TimelineSignal = (function (Tone) {
    ...
    Object.defineProperty(Tone.TimelineSignal.prototype, 'value', {
      get() {
        const now = this.now();
        const val = this.getValueAtTime(now);
    ...
    Tone.TimelineSignal.prototype.exponentialRampToValueAtTime = function (value, endTime) {
      endTime = this.toSeconds(endTime);
      const beforeEvent = this._searchBefore(endTime);
      if (beforeEvent && beforeEvent.value === 0) {
        this.setValueAtTime(this._minOutput, beforeEvent.time);
      }
      value = this._fromUnits(value);
      const setValue = Math.max(value, this._minOutput);
      this._events.add({
    ...
    Tone.TimelineSignal.prototype.setValueCurveAtTime = function (values, startTime, duration, scaling) {
      ...
      const floats = new Array(values.length);
      ...
      for (let j = 1; j < floats.length; j++) {
        const segmentTime = startTime + j / (floats.length - 1) * duration;
    ...
    Tone.TimelineSignal.prototype.setRampPoint = function (time) {
      time = this.toSeconds(time);
      const val = this._toUnits(this.getValueAtTime(time));
      const before = this._searchBefore(time);
      if (before && before.time === time) {
        ...
        const after = this._searchAfter(time);
        if (after) {
          this.cancelScheduledValues(time);
    ...
    Tone.TimelineSignal.prototype._curveInterpolate = function (start, curve, duration, time) {
      ...
      if (time >= start + duration) {
        ...
        const lowerIndex = Math.floor((len - 1) * progress);
        const upperIndex = Math.ceil((len - 1) * progress);
        const lowerVal = curve[lowerIndex];
        const upperVal = curve[upperIndex];
  ...
  env = (function () {
    ...
    const Mult = Tone_signal_Multiply;
    const Scale = Tone_signal_Scale;
    const TimelineSignal = Tone_signal_TimelineSignal;
    const Tone = Tone_core_Tone;
    Tone.setContext(p5sound.audiocontext);
    /**
   ...
   */
    p5.Env.prototype.play = function (unit, secondsFromNow, susTime) {
      const tFromNow = secondsFromNow || 0;
   ...
   */
    p5.Env.prototype.ramp = function (unit, secondsFromNow, v1, v2) {
      ...
      const tFromNow = secondsFromNow || 0;
      let t = now + tFromNow;
      const destination1 = this.checkExpInput(v1);
      const destination2 = typeof v2 !== 'undefined' ? this.checkExpInput(v2) : undefined;
      ...
      // get current value
      const currentVal = this.checkExpInput(this.control.getValueAtTime(t));
      // this.control.cancelScheduledValues(t);
      // if it's going up
   ...
   */
    p5.Env.prototype.add = function (num) {
      const add = new Add(num);
      const thisChain = this.mathOps.length;
      const nextChain = this.output;
   ...
   */
    p5.Env.prototype.scale = function (inMin, inMax, outMin, outMax) {
      const scale = new Scale(inMin, inMax, outMin, outMax);
      const thisChain = this.mathOps.length;
      const nextChain = this.output;
   ...
   */
    p5.Pulse = function (freq, w) {
      ...
      this.f = freq || 440;
      const mW = this.w / this.oscillator.frequency.value;
   ...
   */
    ...
    function createDCOffset() {
      const ac = p5sound.audiocontext;
      const buffer = ac.createBuffer(1, 2048, ac.sampleRate);
      const data = buffer.getChannelData(0);
      for (let i = 0; i < 2048; i++) { data[i] = 1; }
   ...
   */
    ...
    var _whiteNoise = (function () {
      const bufferSize = 2 * p5sound.audiocontext.sampleRate;
      const whiteBuffer = p5sound.audiocontext.createBuffer(1, bufferSize, p5sound.audiocontext.sampleRate);
      const noiseData = whiteBuffer.getChannelData(0);
    ...
    var _pinkNoise = (function () {
      const bufferSize = 2 * p5sound.audiocontext.sampleRate;
      const pinkBuffer = p5sound.audiocontext.createBuffer(1, bufferSize, p5sound.audiocontext.sampleRate);
      const noiseData = pinkBuffer.getChannelData(0);
    ...
    var _brownNoise = (function () {
      const bufferSize = 2 * p5sound.audiocontext.sampleRate;
      const brownBuffer = p5sound.audiocontext.createBuffer(1, bufferSize, p5sound.audiocontext.sampleRate);
      const noiseData = brownBuffer.getChannelData(0);
      let lastOut = 0;
      for (let i = 0; i < bufferSize; i++) {
        const white = Math.random() * 2 - 1;
   ...
   */
    p5.AudioIn.prototype.start = function (successCallback, errorCallback) {
      ...
      const audioSource = p5sound.inputSources[self.currentSource];
      const constraints = {
        audio: {
    ...
    p5.AudioIn.prototype.getSources = function (onSuccess, onError) {
      return new Promise(((resolve, reject) => {
        window.navigator.mediaDevices.enumerateDevices().then((devices) => {
          p5sound.inputSources = devices.filter((device) => {
  ...
  /** Tone.js module by Yotam Mann, MIT License 2016  http://opensource.org/licenses/MIT * */
  let Tone_signal_GreaterThanZero;
  Tone_signal_GreaterThanZero = (function (Tone) {
    'use strict';

    Tone.GreaterThanZero = function () {
      this._thresh = this.output = new Tone.WaveShaper(((val) => {
        if (val <= 0) {
          return 0;
        } else {
          return 1;
  ...
  Tone_signal_Abs = (function (Tone) {
    ...
    Tone.Abs = function () {
      this._abs = this.input = this.output = new Tone.WaveShaper(((val) => {
        if (val === 0) {
          return 0;
        } else {
          return Math.abs(val);
  ...
  Tone_signal_Modulo = (function (Tone) {
    ...
    Tone.Modulo.prototype._setWaveShaper = function (mod) {
      this._shaper.setMap((val) => {
        const multiple = Math.floor((val + 0.0001) / mod);
        return multiple;
      });
    };
  ...
  /** Tone.js module by Yotam Mann, MIT License 2016  http://opensource.org/licenses/MIT * */
  let Tone_signal_AudioToGain;
  Tone_signal_AudioToGain = (function (Tone) {
    'use strict';

    Tone.AudioToGain = function () {
      this._norm = this.input = this.output = new Tone.WaveShaper(((x) => {
        return (x + 1) / 2;
      }));
    };
  ...
  /** Tone.js module by Yotam Mann, MIT License 2016  http://opensource.org/licenses/MIT * */
  let Tone_signal_Expr;
  Tone_signal_Expr = (function (Tone) {
    'use strict';

    Tone.Expr = function () {
      const expr = this._replacements(Array.prototype.slice.call(arguments));
      const inputCount = this._parseInputs(expr);
      this._nodes = [];
      this.input = new Array(inputCount);
      for (let i = 0; i < inputCount; i++) {
        this.input[i] = this.context.createGain();
      }
      const tree = this._parseTree(expr);
      let result;
      try {
    ...
    Tone.Expr._Expressions = {
      value: {
        signal: {
          regexp: /^\d+\.\d+|^\d+/,
          method(arg) {
            const sig = new Tone.Signal(getNumber(arg));
            return sig;
          },
        },
        input: {
      ...
      func: {
        ...
        mod: {
          regexp: /^mod/,
          method(args, self) {
            const modulus = literalNumber(args[1]);
            const op = new Tone.Modulo(modulus);
        ...
        pow: {
          regexp: /^pow/,
          method(args, self) {
            const exp = literalNumber(args[1]);
            const op = new Tone.Pow(exp);
            self._eval(args[0]).connect(op);
            return op;
          },
        },
    ...
    Tone.Expr.prototype._parseInputs = function (expr) {
      const inputArray = expr.match(/\$\d/g);
      let inputMax = 0;
      if (inputArray !== null) {
        for (let i = 0; i < inputArray.length; i++) {
          const inputNum = parseInt(inputArray[i].substr(1)) + 1;
    ...
    Tone.Expr.prototype._tokenize = function (expr) {
      ...
      while (expr.length > 0) {
        expr = expr.trim();
        const token = getNextToken(expr);
        tokens.push(token);
        expr = expr.substr(token.value.length);
      }
      function getNextToken(expr) {
        for (const type in Tone.Expr._Expressions) {
          const group = Tone.Expr._Expressions[type];
          for (const opName in group) {
            const op = group[opName];
            const reg = op.regexp;
            const match = expr.match(reg);
    ...
    Tone.Expr.prototype._parseTree = function (expr) {
      const lexer = this._tokenize(expr);
      const isUndef = this.isUndef.bind(this);
      function matchSyntax(token, syn) {
        return !isUndef(token) && token.type === 'glue' && token.value === syn;
      }
      function matchGroup(token, groupName, prec) {
        const ret = false;
        const group = Tone.Expr._Expressions[groupName];
        if (!isUndef(token)) {
          for (const opName in group) {
            const op = group[opName];
            if (op.regexp.test(token.value)) {
    ...
    Tone.Expr.prototype._eval = function (tree) {
      if (!this.isUndef(tree)) {
        const node = tree.method(tree.args, this);
  ...
  /** Tone.js module by Yotam Mann, MIT License 2016  http://opensource.org/licenses/MIT * */
  let Tone_signal_EqualPowerGain;
  Tone_signal_EqualPowerGain = (function (Tone) {
    'use strict';

    Tone.EqualPowerGain = function () {
      this._eqPower = this.input = this.output = new Tone.WaveShaper(((val) => {
  ...
  effect = (function () {
    const p5sound = master;
    const CrossFade = Tone_component_CrossFade;
    /**
  ...
  */
    p5.Effect.prototype.connect = function (unit) {
      const u = unit || p5.soundOut.input;
  ...
  */
   ...
   *  @private
   */
    const EQFilter = function (freq, res) {
   ...
   */
    ...
    // if loading locally without a server
      ...
      const cReverb = new p5.Convolver(path, callback, errorCallback);
      cReverb.impulses = [];
      return cReverb;
    };
    /**
    ...
    p5.Convolver.prototype._loadBuffer = function (path, callback, errorCallback) {
      ...
      request.onload = function () {
        ...
        // on success loading file:
          ...
          const err = new CustomError('loadConvolver', errorTrace, self.url);
          const msg = `Unable to load ${self.url}. The request status was: ${request.status} (${request.statusText})`;
          if (errorCallback) {
            err.message = msg;
            errorCallback(err);
  ...
  Tone_core_Clock = (function (Tone) {
    ...
    Tone.Clock.prototype._loop = function () {
      ...
      const updateInterval = this.context.updateInterval;
      const lagCompensation = this.context.lag * 2;
      const loopInterval = now + lookAhead + updateInterval + lagCompensation;
      while (loopInterval > this._nextTick && this._state) {
        const currentState = this._state.getValueAtTime(this._nextTick);
        if (currentState !== this._lastState) {
          this._lastState = currentState;
          const event = this._state.get(this._nextTick);
  ...
  metro = (function () {
    ...
    p5.Metro.prototype.ontick = function (tickTime) {
      const elapsedTime = tickTime - this.prevTick;
      const secondsFromNow = tickTime - p5sound.audiocontext.currentTime;
      ...
      // console.log('ok', this.syncedParts[0].phrases[0].name);
        ...
        const self = this;
        this.syncedParts.forEach((thisPart) => {
          if (!thisPart.isPlaying) { return; }
          thisPart.incrementStep(secondsFromNow);
          // each synced source keeps track of its own beat number
          thisPart.phrases.forEach((thisPhrase) => {
            const phraseArray = thisPhrase.sequence;
            const bNum = self.metroTicks % phraseArray.length;
    ...
    p5.Metro.prototype.setBPM = function (bpm, rampTime) {
      const beatTime = 60 / (bpm * this.tatums);
      const now = p5sound.audiocontext.currentTime;
   ...
   */
    ...
    p5.Score.prototype.resetPart = function (i) {
      ...
      for (const p in this.parts[i].phrases) {
        if (this.parts[i]) {
   ...
   */
    ...
    * true if string, false if number
       ...
       */
      this.musicalTimeMode = typeof this._interval !== 'number';
     ...
     */
      ...
      this.clock = new Clock({
        callback(time) {
          const timeFromNow = time - p5sound.audiocontext.currentTime;
          /**
   ...
   */
    p5.SoundLoop.prototype.syncedStart = function (otherLoop, timeFromNow) {
      ...
      if (!otherLoop.isPlaying) {
        ...
        const time = otherLoop.clock._nextTick - p5sound.audiocontext.currentTime;
   ...
   */
    Object.defineProperty(p5.SoundLoop.prototype, 'interval', {
      ...
      set(interval) {
        this.musicalTimeMode = typeof interval !== 'Number';
  ...
  */
   ...
   */
    ...
    p5.SoundRecorder.prototype._mergeBuffers = function (channelBuffer) {
      const result = new Float32Array(this.recordedSamples);
      let offset = 0;
   ...
   */
    p5.prototype.saveSound = function (soundFile, name) {
      ...
      const buffer = new window.ArrayBuffer(44 + interleaved.length * 2);
   ...
   */
    p5.PeakDetect.prototype.update = function (fftObject) {
      const nrg = this.energy = fftObject.getEnergy(this.f1, this.f2) / 255;
   ...
   */
    p5.AudioVoice.prototype._setNote = function (note) {
      ...
      let value = wholeNotes[note[0]];
      const octave = typeof Number(note.slice(-1)) === 'number' ? note.slice(-1) : 0;
      value += 12 * octave;
  ...
  * */
   ...
   */
    p5.MonoSynth.prototype.triggerAttack = function (note, velocity, secondsFromNow) {
      ...
      const freq = typeof note === 'string' ? this._setNote(note) : typeof note === 'number' ? note : 440;
  ...
  * */
   ...
   */
    p5.PolySynth.prototype._allocateVoices = function () {
      for (let i = 0; i < this.polyValue; i++) {
   ...
   */
    p5.PolySynth.prototype.noteAttack = function (_note, _velocity, secondsFromNow) {
      ...
      const note = typeof _note === 'string' ? this.AudioVoice.prototype._setNote(_note) : typeof _note === 'number' ? _note : 440;
      let velocity = _velocity === undefined ? 1 : _velocity;
      let currentVoice;
      ...
      if (this._voicesInUse.getValueAtTime(t) < this.polyValue) {
        ...
        const oldestNote = p5.prototype.freqToMidi(this.audiovoices[this._oldest].oscillator.freq().value);
      ...
      const previousVal = this._voicesInUse._searchBefore(t) === null ? 0 : this._voicesInUse._searchBefore(t).value;
      ...
      if (typeof velocity === 'number') {
        const maxRange = 1 / this._voicesInUse.getValueAtTime(t) * 2;
   ...
   */
    p5.PolySynth.prototype._updateAfter = function (time, value) {
      if (this._voicesInUse._searchAfter(time) === null) {
        ...
        const nextTime = this._voicesInUse._searchAfter(time).time;
   ...
   */
    ...
    // Make sure note is in frequency inorder to query the this.notes object
      const note = typeof _note === 'string' ? this.AudioVoice.prototype._setNote(_note) : typeof _note === 'number' ? _note : this.audiovoices[this._newest].oscillator.freq().value;
      const now = p5sound.audiocontext.currentTime;
      const tFromNow = secondsFromNow || 0;
      const t = now + tFromNow;
  ...
  */
   ...
   */
    p5.Distortion = function (amount, oversample) {
      ...
      const curveAmount = p5.prototype.map(amount, 0, 1, 0, 2000);
      /**
